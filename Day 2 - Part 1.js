  ...
  const gameId = input.split(':')[0];
	...
	roundData.forEach(round => {
      ...
      const assignNumber = Number(colour.split(' ')[0])
			if (colour.includes('red')) red = assignNumber;
			if (colour.includes('blue')) blue = assignNumber
			if (colour.includes('green')) green = assignNumber;
		})
	if (red > 12 || green > 13 || blue > 14) roundResults.push("Invalid")
	})
if (!roundResults.includes('Invalid')) validRounds.push(Number(gameId.split(' ')[1]))
})


validRounds.reduce((acc, val) => acc +val)
