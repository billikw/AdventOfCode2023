...
let cubedResults = [];
let highestColourCounts = [];

formattedInput.forEach(input => {
  const gameId = input.split(':')[0];
	const gameData = input.split(': ')[1];
	const roundData = gameData.split(';');
	let roundResults = [];
	let highestColourCount = {"red": 0, "green": 0, "blue": 0};
  
    	roundData.forEach(round => {
    			let red = {};
    			let blue = {};
    			let green = {};
    		round.match(/([0-9]+) (red|green|blue)/g).forEach(colour => {
          const assignNumber = Number(colour.split(' ')[0]);
    			if (colour.includes('red')) red = {"red": assignNumber};
    			if (colour.includes('blue')) blue = {"blue": assignNumber};
    			if (colour.includes('green')) green = {"green": assignNumber};
    			roundResults.push(red, green, blue);
    		})
    	roundResults.forEach(round => {
    		if (round.red > highestColourCount.red) highestColourCount.red = round.red;
    		if (round.green > highestColourCount.green) highestColourCount.green = round.green;	
    		if (round.blue > highestColourCount.blue) highestColourCount.blue = round.blue;
    	})
	  })
  highestColourCounts.push(highestColourCount);
})

cubedResults = highestColourCounts.map(x => {
  return x.red * x.green * x.blue;
})

cubedResults.reduce((acc, val) => acc + val);
// Result: 66909.
